"""RQ2 classifiers: pose-feature MLP vs appearance-only CNN.

Both share the same training/eval loop so the comparison is apples-to-apples:
same splits, same optimizer settings, same metrics (macro-F1, per-class F1,
confusion matrix).
"""
...
class AppearanceCNN(nn.Module):
    """Small CNN on 64x64 RGB person crops."""
...
def train_classifier(model, X_train, y_train, X_val, y_val, *,
    ...
    """Shared loop. X are float32 tensors/arrays; y are int64 label indices.

    Keeps the best model by val macro-F1; returns (model, history).
    """
    ...
    for epoch in range(epochs):
        ...
        if verbose and epoch % 5 == 0:
            print(f"  epoch {epoch:3d} loss {total/n:.4f} val macro-F1 {f1:.3f}")
        if f1 > best_f1:
...
def evaluate(y_true, y_pred, class_names, tag, save_fig=True):
    """Print report, save confusion-matrix figure, return metrics dict."""
